...
function toXlf2(translationFile, options) {
    ...
    file.children = translationFile.units.map(unit => {
        var _a;
        const u = new xmldoc_1.XmlDocument(`<unit id="${unit.id}"><segment><source>${unit.source}</source></segment></unit>`);
...
function toXlf1(translationFile, options) {
    ...
    body.children = translationFile.units.map(unit => {
        var _a;
        const transUnit = new xmldoc_1.XmlDocument(`<trans-unit id="${unit.id}" datatype="html">
